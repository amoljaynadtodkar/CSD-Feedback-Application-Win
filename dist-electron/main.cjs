...
function getPreloadPath() {
  if (isDev()) {
    return import_path.default.join(import_electron.app.getAppPath(), "./dist-electron/preload.cjs");
  } else {
    return import_path.default.join(import_electron.app.getAppPath(), "dist-electron", "preload.cjs");
  }
}
...
function getLogStream() {
  if (!logStream) {
    const logPath = (0, import_path2.join)(
      os2.homedir(),
      "Library",
      "Application Support",
...
function getConfigPath() {
  ...
  return (0, import_path2.join)(userDataPath, "server-config.json");
}
...
function saveServerConfig(config) {
  try {
    const configPath = getConfigPath();
    fs2.mkdirSync((0, import_path2.dirname)(configPath), { recursive: true });
...
function waitForBackend(url, maxRetries = 30, interval = 1e3) {
...
import_electron4.app.on("ready", async () => {
  ...
  import_electron4.ipcMain.handle("getServerConfig", () => readServerConfig());
  import_electron4.ipcMain.handle("saveServerConfig", (_event, newConfig) => {
